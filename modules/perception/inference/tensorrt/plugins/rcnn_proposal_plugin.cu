...
int RCNNProposalPlugin::enqueue(int batchSize, const void *const *inputs,
  ...
  BASE_CUDA_CHECK(cudaMemcpyAsync(result_boxes, init_result_boxes,
  ...
  for (int batch_id = 0; batch_id < batchSize; ++batch_id) {
    ...
    // TODO(cjh): need to track class_id for the output
  ...
  // TODO(chenjiahao): output all classification scores
