...
__global__ void bbox_transform_inv_kernel(const int nthreads,
                                          const float *boxes,
                                          const float *deltas,
                                          const int num_box,
...
//  float x_ctr = x_min + 0.5 * w;
//  float y_ctr = y_min + 0.5 * h;
  // TODO(chenjiahao): testing
  ...
  // TODO(chenjiahao): testing
//  out_boxes[index * 4] = pred_x_ctr - 0.5 * pred_w;  // pred x_min
//  out_boxes[index * 4 + 1] = pred_y_ctr - 0.5 * pred_h;  // pred y_min
//  out_boxes[index * 4 + 2] = pred_x_ctr + 0.5 * pred_w;  // pred x_max
//  out_boxes[index * 4 + 3] = pred_y_ctr + 0.5 * pred_h;  // pred y_max
...
// boxes dims: [N, num_box, num_channel, 4], filtered_boxes dims: [N, num_box, 4],
// scores dims: [N, num_box, num_class], filtered_scores dims: [N, num_box],
// filtered_count dims: [N]
__global__ void filter_boxes_kernel(const int nthreads, const float *boxes,
                                    const float *scores, const int num_box,
                                    const int num_channel,
                                    const int num_class,
...
__global__ void keep_topN_boxes_kernel(const int nthreads, const float *boxes,
                                       const float *scores, const int *indexes,
                                       const int *count, const bool keep_score,
                                       const int num_box, const int topN,
...
//__global__ void repeatedly_div_kernel(const int nthreads, const float *in_data,
//                                      float *out_data, const float *div_vec,
//                                      int div_vec_size) {
//  int index = threadIdx.x + blockIdx.x * blockDim.x;
//  if (index < nthreads) {
//    out_data[index] = in_data[index] / div_vec[index % div_vec_size];
//  }
//}
...
//void batch_sort_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *in_data,
//                     const int len, const int *sort_len,
//                     int *sorted_indexes) {
//  batch_sort_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, in_data,
//                                                                     len, sort_len,
//                                                                     sorted_indexes);
//}

void bbox_transform_inv_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads,
                             const float *boxes,
                             const float *deltas,
                             const int num_box,
                             const int num_channel,
                             float *out_boxes) {
  bbox_transform_inv_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads,
                                                                             boxes,
                                                                             deltas,
                                                                             num_box,
                                                                             num_channel,
...
void clip_boxes_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, float *boxes,
                     const float height, const float width) {
  clip_boxes_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, boxes,
                                                                     height, width);
}

void filter_boxes_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *boxes,
                       const float *scores, const int num_box,
                       const int num_channel,
                       const int num_class,
                       const int filter_channel,
                       ...
                       float *filtered_scores,
                       int *filtered_count) {
  filter_boxes_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, boxes,
                                                                       scores, num_box,
                                                                       num_channel,
                                                                       num_class,
                                                                       filter_channel,
...
void keep_topN_boxes_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *boxes,
                          const float *scores, const int *indexes,
                          const int *count, const bool keep_score,
                          const int num_box, const int topN,
                          float *out_boxes, float *out_scores) {
  keep_topN_boxes_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, boxes,
                                                                          scores, indexes,
                                                                          count, keep_score,
                                                                          num_box, topN,
...
void repeatedly_add_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *in_data,
                         float *out_data, const float *add_vec,
                         int add_vec_size) {
  repeatedly_add_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, in_data,
                                                                         out_data, add_vec,
                                                                         add_vec_size);
}

//void repeatedly_div_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *in_data,
//                         float *out_data, const float *div_vec,
//                         int div_vec_size) {
//  repeatedly_div_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, in_data,
//                                                                         out_data, div_vec,
//                                                                         div_vec_size);
//}

void repeatedly_mul_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *in_data,
                         float *out_data, const float *mul_vec,
                         int mul_vec_size) {
  repeatedly_mul_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, in_data,
                                                                         out_data, mul_vec,
                                                                         mul_vec_size);
}

void slice2d_cuda(int block_size, int thread_size, int shared_mem, cudaStream_t stream, const int nthreads, const float *in_data,
                  float *out_data, const int *slice_axises,
                  int slice_axis_num, int input_axis_size) {
  slice2d_kernel<<<block_size, thread_size, shared_mem, stream>>>(nthreads, in_data,
                                                                  out_data, slice_axises,
                                                                  slice_axis_num, input_axis_size);
}
