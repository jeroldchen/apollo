...
// boxes dims: [topN, 4], scores dims: [topN]
...
__global__ void convert_out_boxes_kernel(const int nthreads, const float *boxes,
  ...
  if (index < nthreads) {
    int out_channel = rpn_proposal_output_score ? (5 + num_class) : 5;
...
void NmsForward(const bool rpn_proposal_output_score,
    ...
    const float nms_overlap_threshold, const int num_candidate, const int top_n,
    const int batch_id, const int class_id, float *dev_sorted_box_for_nms,
    float *scores, float *out_boxes, int *acc_box_num, cudaStream_t stream) {
  ...
  nms_kernel<<<blocks, threads, 0, stream>>>(host_filter_count,
      nms_overlap_threshold, dev_sorted_box_for_nms, dev_mask, num_box_corners);

  // postprocess for nms output
  std::vector<uint64_t> host_mask(host_filter_count * col_blocks);
  BASE_CUDA_CHECK(cudaMemcpyAsync(&host_mask[0], dev_mask,
  ...
  BASE_CUDA_CHECK(cudaMemcpyAsync(dev_keep_num, &out_num_to_keep, sizeof(int), cudaMemcpyHostToDevice, stream));
#if DEBUG
  std::cout << " out_num_to_keep: " << out_num_to_keep;
#endif
  int nthreads = top_n;
  int block_size = (nthreads - 1) / NUM_THREADS_MACRO + 1;
  keep_topN_boxes_cuda(block_size, NUM_THREADS_MACRO, 0, stream, nthreads,
      dev_sorted_box_for_nms, scores, dev_keep_inds, dev_keep_num, rpn_proposal_output_score, num_candidate, top_n,
  ...
  convert_out_boxes_kernel<<<block_size, NUM_THREADS_MACRO, 0, stream>>>(
      nthreads, kept_boxes, kept_scores, 4, batch_id, class_id, rpn_proposal_output_score,
      out_boxes);
#if DEBUG
  BASE_CUDA_CHECK(cudaDeviceSynchronize());
#endif
